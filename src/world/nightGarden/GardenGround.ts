...
export class GardenGround {
  ...
  constructor(parent: Group, material: MeshStandardMaterial) {
    ...
    for (let index = 0; index < values.length; index += 3) {
      ...
      const pondDistance = Math.hypot((x - 2.75) / 6.15, (worldZ + 25.2) / 6.8)
      const terrain = Math.sin(x * 0.45 + localZ * 0.18) * 0.075 + Math.cos(localZ * 0.56 - x * 0.14) * 0.045
      const shoreLift = Math.exp(-Math.abs(pondDistance - 1) * 8) * 0.095
      const pondBasin = pondDistance < 1 ? -0.13 * Math.pow(1 - pondDistance, 1.4) : 0
      const nearWeight = Math.max(0, Math.min(1, (-localZ - 17) / 8))
      values[index + 1] += nearWeight * (1.45 + Math.sin(x * 0.41) * 0.82 + Math.cos(x * 0.19) * 0.42)
      values[index + 2] = terrain + shoreLift + pondBasin

      const source = pondDistance < 1.3 ? deep : (Math.sin(x * 0.18 + worldZ * 0.09) > 0.15 ? damp : pathSoil)
      const tone = 0.78 + Math.min(0.18, shoreLift * 1.1) + Math.sin(x * 0.26 + worldZ * 0.21) * 0.045
